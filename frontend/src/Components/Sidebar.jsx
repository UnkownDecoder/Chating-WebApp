...
const Sidebar = ({ toggleFriendsView }) => {
  ...
  const handleFriendClick = (friend) => {
    setSelectedUser(selectedUser?._id === friend._id ? null : friend);
    ...
    <div id="sidebar" className={`fixed inset-0 md:relative md:w-64 bg-gray-900 text-white p-4 flex flex-col ${isSidebarVisible ? 'block' : 'hidden md:block'}`}>
      ...
      <div className="mt-2 flex-1 overflow-y-auto">
        {isFetchingFriends ? (
          ...
          filteredFriends.map((friend) => (
            <div
              key={friend._id}
              className={`flex items-center p-2 rounded-lg mb-2 cursor-pointer ${selectedUser?._id === friend._id ? "bg-blue-600" : "bg-gray-800 hover:bg-gray-700"}`}
        ...
        {groups.length > 0 && (
          <div className="mt-4">
            ...
            {groups.map((group) => (
              <div
                key={group._id}
                className={`flex items-center p-2 rounded-lg mb-2 cursor-pointer ${selectedUser?._id === group._id ? "bg-blue-600" : "bg-gray-800 hover:bg-gray-700"}`}
      ...
      <div className="absolute bottom-0 left-0 w-full bg-gray-900 p-2 flex items-center justify-between border-t border-gray-700 h-16 sm:h-20 z-30 md:flex">
        ...
        <div className="flex space-x-3">
          ...
          <button className="p-2 text-white hover:text-gray-400" onClick={handleSettingsClick}><FaCog size={16} /></button>
        </div>
      </div>
  ...
  <CreateGroupPopup 
    ...
    onCreateGroup={(groupData) => {
      createGroup(groupData); // Call createGroup with groupData
